...
final class PreviewMediaRequestTests: XCTestCase {
    ...
    /// register — and `URLSession` forwards a manually-set header to whatever host it is given.
    /// Unreachable via anything the current server mints, which is why it is worth pinning: it
    /// turns on what the OTHER end of the wire does.
    func testRelativePathWithoutALeadingSlashIsRefusedRatherThanChangingHost() {
        XCTAssertNil(
            LurkerClient.mediaRequest(
                for: "api/link-preview/media/abc", baseURL: "https://chat.example", token: "t"))
    }
